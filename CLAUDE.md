...
- **Verification commands**: default to running `pnpm test`/`lint`/`typecheck` etc. at the turborepo root (Turborepo scopes/caches per-package on its own), then confirm the output actually covered the intended package(s) — rather than reaching for `--filter @vc/x` on every command. Revisit this default if/when the number of packages grows enough that root-level runs become too slow/costly to use routinely.
