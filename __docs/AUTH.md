...
- **How a new real user gets an `org_id` on first `SupabaseSessionResolver` login** (`User.org_id` is required per `DATA_MODEL.md` but nothing assigns it for non-demo users yet). Scoped to org assignment only — a user belongs to exactly one org/league, and that's intentionally the locked, 1:1 layer; this is separate from `TeamMembership`, which `DATA_MODEL.md` already specifies as easy, self-service, many-to-many (join/leave freely, defaults to `volunteer` role) and isn't part of this decision. Two candidate shapes for org assignment, not yet decided between:
  1. Invite-only — an existing org admin issues an invite (a code to paste, or a URL carrying it) that resolves to a specific `org_id` at signup time.
  2. Self-serve — the new user searches/selects a target org, and joins in a pending state until that org's admin approves.
     Whichever is picked determines how `OrgRole` gets its initial row; `TeamMembership` provisioning is unaffected and stays a separate, later, self-service step once the user is in an org.
