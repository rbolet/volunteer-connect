...
**OrgInvite** (schema + Express layer built — `apps/api/src/repositories/org-invites.ts`; not yet in `apps/web`, see `__docs/plans/REAL_AUTH_IMPLEMENTATION.md` Phase 2+. **Migration not yet applied to any DB** — no local dev Postgres exists yet, see `__docs/NEXT_SESSION.md`.)
...
- No admin-facing UI to issue invites yet — rows are created via `packages/db/scripts/create-invite.ts` for UAT. Promoting a user to org admin is a direct DB edit for now, not an in-app action.
