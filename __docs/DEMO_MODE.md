...
**Status: implemented for the initial demo (2026-07-15), except the reset cron job.** Seed generator (`packages/db/src/seed/`: `generate.ts` + `run.ts` + `reset-demo.ts`/`wipe.ts`; `pnpm --filter @vc/db db:seed` / `db:reset-demo`), demo entry route + banner + "view as" switcher (`apps/web/src/app/demo/`), and the `DemoSessionResolver` path (AUTH.md) are live; the demo org is seeded in the live Supabase DB. Still outstanding: the scheduled Railway cron reset (script exists, scheduling doesn't) and RLS (below). The seed content is hand-curated in `demo-data.ts`, not faker-generated — determinism section below predates that choice.
...
**Current reality vs. this section's original wording (as of the 2026-07-23 `app/demo/` reorg):** the paragraph below describes `/demo` as a thin entry point that redirects into a separate normal app shell — that's the _target_ end state, not what exists today. Right now there is no separate app shell to redirect into: `/demo/*` itself is the only route tree (`/demo/dashboard`, `/demo/signups`, `/demo/teams`, `/demo/signup-templates`), and it stays that way until `SupabaseSessionResolver` is real. What _did_ land in the 2026-07-23 reorg is the file-layout half of this: `apps/web/src/app/demo/` now holds only genuinely demo-only code (banner, "view as" switcher, unseeded-org fallback) plus thin route glue, while all actual UI/business logic moved to `apps/web/src/features/` — so when the redirect-based entry point described below is eventually built, it reuses that code rather than duplicating it. Full phased build-out plan: `__docs/plans/REAL_AUTH_IMPLEMENTATION.md`. Directory layout: `__docs/FRONTEND.md`.
